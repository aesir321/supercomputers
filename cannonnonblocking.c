...
int main (int argc, char **argv) {
	...
	double *A_receive_buffer = NULL, *B_receive_buffer = NULL;
	...
	A_receive_buffer = (double *) malloc (A_local_block_size * sizeof(double));
	...
	B_receive_buffer = (double *) malloc (A_local_block_size * sizeof(double));
...
MPI_Sendrecv_replace(B_local_block,B_local_block_size,MPI_DOUBLE,columndest,2,columnsource,2,column_communicator,&status);
	...
	double compute_time = 0, mpi_time = 0, inner_mpi_time = 0, start_comp, start_mpi;
	int C_index, A_row, A_column, B_column;
	MPI_Request A_send_request = MPI_REQUEST_NULL, recv_request_A = MPI_REQUEST_NULL;
	MPI_Request B_send_request = MPI_REQUEST_NULL, recv_request_B = MPI_REQUEST_NULL;
	int test_A_send_request, test_B_send_request;
	int A_posted_send, B_posted_send;

//first cycle doesnt need to recieve a local block
	start_mpi = MPI_Wtime();

		MPI_Isend(A_local_block, A_local_block_size, MPI_DOUBLE,
				  (coordinates[1] + sqrt_size - 1) % sqrt_size, 0,
				  row_communicator, &A_send_request);


		MPI_Irecv(A_receive_buffer, A_local_block_size, MPI_DOUBLE,
		...
		MPI_Isend(B_local_block, B_local_block_size, MPI_DOUBLE,
				  ...
				  column_communicator, &B_send_request);


		MPI_Irecv(B_receive_buffer, B_local_block_size, MPI_DOUBLE,
	...
	for(cannon_block_cycle = 1; cannon_block_cycle < sqrt_size - 1; cannon_block_cycle++){
		...
		start_comp = MPI_Wtime();
			...
			for(C_index = 0, A_row = 0; A_row < A_local_block_rows; A_row++){
				...
				start_mpi = MPI_Wtime();
					//if the the send has been fulfilled, give it a new value to save time
					if (!A_posted_send) {
						MPI_Test(&A_send_request, &test_A_send_request, MPI_STATUS_IGNORE);
						if (test_A_send_request) {
							MPI_Isend(A_local_block, A_local_block_size, MPI_DOUBLE,
									  (coordinates[1] + sqrt_size - 1) % sqrt_size, 0,
									  row_communicator, &A_send_request);
							A_posted_send = 1;
					...
					if (!B_posted_send) {
						MPI_Test(&B_send_request, &test_B_send_request, MPI_STATUS_IGNORE);
						if (test_B_send_request) {
							MPI_Isend(B_local_block, B_local_block_size, MPI_DOUBLE,
									  (coordinates[0] + sqrt_size - 1) % sqrt_size, 0,
									  column_communicator, &B_send_request);
							B_posted_send = 1;
				...
				inner_mpi_time += MPI_Wtime() - start_mpi;
			}
		compute_time += MPI_Wtime() - start_comp - inner_mpi_time;
		mpi_time += inner_mpi_time;

		start_mpi = MPI_Wtime();
		//by the end of the iteration at least one send pair has to be forced
			if (!A_posted_send) {
				MPI_Wait(&A_send_request, MPI_STATUS_IGNORE);
				MPI_Isend(A_local_block, A_local_block_size, MPI_DOUBLE,
						  (coordinates[1] + sqrt_size - 1) % sqrt_size, 0,
						  row_communicator, &A_send_request);
				A_posted_send = 1;
...
//no need for new sends at the last interation
	start_mpi = MPI_Wtime();
		 MPI_Wait(&recv_request_A ,&status);
		 MPI_Wait(&recv_request_B ,&status);
	mpi_time += MPI_Wtime() - start_mpi;
	...
	free(A_receive_buffer);
	free(B_receive_buffer);
